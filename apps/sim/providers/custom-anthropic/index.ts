...
import { parseCustomAnthropicModel } from '@/providers/custom-anthropic/utils'
...
const logger = createLogger('CustomAnthropicProvider')
const CUSTOM_ANTHROPIC_VERSION = '1.0.0'

export const customAnthropicProvider: ProviderConfig = {
  id: 'custom-anthropic',
  name: 'Custom Anthropic',
  description: 'Custom Anthropic-compatible endpoints',
  version: CUSTOM_ANTHROPIC_VERSION,
  ...
  executeRequest: async (
    ...
    const { endpointName, modelName } = parseCustomAnthropicModel(request.model)

    logger.info('Preparing Custom Anthropic request', {
      model: request.model,
      endpointName,
      modelName,
    ...
    const baseUrl = request.customAnthropicBaseUrl
    if (!baseUrl) {
      throw new Error(
        `Custom Anthropic endpoint "${endpointName || 'unknown'}" not configured. Please configure the endpoint in workspace settings.`
      )
    }
    ...
    const anthropic = new Anthropic({
      apiKey,
      baseURL: baseUrl,
      defaultHeaders: request.customAnthropicHeaders,
    })
      ...
      try {
        ...
        if (filteredTools?.length) {
          ...
          if (typeof tc === 'object' && tc !== null) {
            if (tc.type === 'tool') {
              toolChoice = tc
              logger.info(`Using tool_choice format: force tool "${tc.name}"`)
            } else {
              toolChoice = 'auto'
    ...
    if (request.responseFormat) {
      ...
      const schemaInstructions = `IMPORTANT: You must respond with a valid JSON object that conforms to the following schema.
Do not include any text before or after the JSON object. Only output the JSON.

Schema name: ${request.responseFormat.name || 'response'}
JSON Schema:
...
Your response must be valid JSON that exactly matches this schema structure.`
    ...
    try {
      if (request.stream && (!anthropicTools || anthropicTools.length === 0)) {
        logger.info('Using streaming response for Custom Anthropic request (no tools)')
      ...
      logger.error('Error in Custom Anthropic request:', {
        error,
        duration: totalDuration,
        endpointName,
      })
...
export { parseCustomAnthropicModel } from '@/providers/custom-anthropic/utils'
