...
export const zaiProvider: ProviderConfig = {
  ...
  executeRequest: async (
    ...
    try {
      ...
      if (tools?.length) {
        preparedTools = prepareToolsWithUsageControl(tools, request.tools, logger, 'zai')
      ...
      if (request.stream && (!tools || tools.length === 0)) {
        ...
        const streamingResult = {
          stream: createReadableStreamFromZAIStream(
      ...
      let currentResponse = await zai.chat.completions.create(payload)
      ...
      try {
        while (iterationCount < MAX_TOOL_ITERATIONS) {
          ...
          currentResponse = await zai.chat.completions.create(nextPayload)
      ...
      if (request.stream) {
        ...
        const streamResponse = await zai.chat.completions.create(streamingPayload)
